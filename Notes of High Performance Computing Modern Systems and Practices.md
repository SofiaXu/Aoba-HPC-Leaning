...
## 第九章 并行算法
### 总结
- 并行算法是一种对于给定应用程序的计算工作的组织方法，是的可以同时执行工作负载的多个部分以减少求解决时间并提高性能。
- 分支-聚合并行性描述了一组可以同时执行的任务，他们从同一个起点开始，然后分支并继续，直到所有任务都完成到达聚合点，才会继续计算。
- 分支-聚合并行性通常在多个物理执行资源之间划分给定的循环的实例。这被称为“循环并行”。
- 分治并行性将一个大问题分成若干个可以同时执行的小问题。同时可以进一步细分每个较小的问题以产生甚至有更小工作的更多并行动作。这种将工作重复划分为更小的子任务的情况会增加应用程序的并行性，知道最小的结果任务易于执行。
- 快速排序是用于排序数据的分支算法的一个例子
- 常规的采样并行排序算法提高了分布式计算的效率和可扩展性，但仍然借助快速排序方法。
- 管理者-工作者工作流有一个进程控制其他工作者进程，这些工作者进程表现出加速总工作负载执行所需的并行性。通过中央控制进程它可以动态地调整负载平衡以适应不断变化的数据状态
- 光环是映射到并行任务数据子集外部的区域。她充当该数据子集的人工边界，并包含源自相邻并行任务数据子集的信息。
- 光环交换使每个人物能够执行计算并更新映射到该任务的数据子集，同时可以访问可能不是本地的但此类计算所需的任何数据。
- 稀疏矩阵计算利用主要由零值元素填充的矩阵，其中仅相对少量的元素是非零的。稀疏数据结构仅通过存储非零元素来压缩矩阵，从而允许表示比计算机的主存储器容量更大的矩阵。
- 任务数据流算法通过有向无环图形式的依赖关系表示子任务之间的先例约束。这确定了在启动后续任务之前必须完成哪些任务。

## 第十章 库
### 总结
- 主要介绍了多种实用并行类库：线性代数（BLAS、Lapack、ScaLapack、GSL、SuperLU、PETSc、SLEPc、Hypre)、偏微分方程（PETSc、Trilinos）、图算法（3PBGL）、并行输入输出（NetCDF、HDF5、Silo）、网格分解（METIS）、可视化（VTK）、并行化（OpenMP、Pthreads）、信号处理（FFTW）、性能监控（PAPI、VampirTrace）

## 第十一章 操作系统
### 总结
- 操作系统（OS）拥有整个超级计算机
- OS 是控制应用程序的持久程序，是用户应用程序和硬件之间的主要接口
- OS 的主要功能是利用一个或多个处理器的硬件资源为系统用户提供一组服务，管理辅助存储设备和 I/O 设备，包括文件系统
- OS 管理的资源包括处理器及其继承内核、内核工作所在系统的贮存、I/O 模块和系统总线
- 正在执行的用户和系统程序由被称为“进程”的实例组成，这些实例时程序过程的实例化
- 单一 OS 下，许多进程可能同时运行。
- OS 内存管理负责程序数据的虚拟地址块（被称为页面）和物理存储块（被称为帧）之间的地址转换
- OS 负责用户的数据和程序，通过命名目录的层次结构组织在文件中
- OS 负责管理进出其支持的计算机中所有数据流的来源和目的地
- OS 负责与系统中所有活动进程相关联的许多服务
- OS 支持的进程管理服务的核心时进程调度的交叉功能：确定为哪些进程提供运行所需的物理资源，以及何时分配这些资源
- 线程在进程中提供了另一种级别的并行控制
- 虚拟内存是一种强大的抽象，独立于内存块的物理位置来命名内存块，操作系统支持这种抽象。
- 操作系统包含一个表业，他是将虚拟页地址（或页号）关联到主内存或辅助存储中的物理位置的方法的关键

## 第十二章 可视化
### 总结
- 介绍了 Gunplot、Matplotlib、VTK、ParaView、VisIt 的基本用法。
- 可视化数据的冬季包括调试和探索数据、统计假设测试和准备演示图形
- 许多科学可视化至少包含 4 中基本的可视化概念中的一种：流线、等值面、通过光纤追踪的体绘制和网格细分
- 流线以矢量场为输入，并显示与矢量场相切的曲线
- 通过光线跟踪的体绘制将光纤投射到数据体中，并对光线经过的体进行采样
- 网格细分使用多边形可视化数据点以及与其他数据点的连接